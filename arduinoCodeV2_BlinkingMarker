...
void loop() {
  ...
  byte markerWidth = 1; //how fat that marker is
  ...
  long markerLocation_mS = sweepTime_mS*log(marker)/log(endFrequency); //calculate how long to wait to place marker
...
//chan2 highlights
    timeElapsed = (micros() - startTime)/1000.;
     ...
     if(blinkMarker == 1){//blink the marker 
       //go ahead and place the marker, to add more just copy the if/else statement below and add a new 'markerLocation_mS' variable. 
       if( timeElapsed >= markerLocation_mS && timeElapsed <= markerLocation_mS+markerWidth)
         digitalWrite(scaleOut,LOW);
       else
         digitalWrite(scaleOut,HIGH);   
     }
